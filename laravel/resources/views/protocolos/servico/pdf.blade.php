...
<body>
    <center>
        ...
        <br><br>
        <p>
            Declaramos que para os devidos fins, que o senhor(a) <i><b>{{ $associado->user[0]->name }}</b></i>,
             portador(a) do <b>CPF: {{ $associado->nb_identificacao }}</b> utilizou-se de seus benefícios, <b>{{ $servico }}</b> na presente Data:
            <b>{{ strftime('%d/%m/%Y às %H:%M hs', strtotime('now'))}}.</b>
        </p>
    </center>
        <p class="pull-right">Manaus, {{  strftime('%A, %d de %B de %Y', strtotime('now')) }}.</p>
        <br><br><br><br>
